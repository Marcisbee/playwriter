    ...
    return (
      <div style={{
        ...
        {recentProjects.length > 0 && (
          <div style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', background: '#fff', borderTop: '2px solid #ccc', padding: 10 }}>
            <h3>Recent Projects:</h3>
            <ul style={{ listStyle: 'none', padding: 0, maxHeight: '300px', overflowY: 'auto' }}>
              {recentProjects.map((proj) => (
                <li key={proj} style={{ marginBottom: '10px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px' }}>
                  <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{proj}</span>
                  <button onClick={() => loadProject(proj)} style={{ marginLeft: 'auto' }}>Load</button>
                  <button onClick={() => removeProject(proj)} title="Remove from list">X</button>
                </li>
              ))}
            </ul>
          </div>
